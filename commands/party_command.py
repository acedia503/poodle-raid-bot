...
from views.party_view import PartyBuildHomeView, build_party_result_embed, build_empty_result_embed
...
class PartyCommand(commands.Cog):
    ...
    @app_commands.command(name="공대", description="현재 채널의 공대 관리 화면을 엽니다.")
    async def party(self, interaction: discord.Interaction):
        try:
            ...
            embed = build_rule_home_embed(rule)
            view = PartyRuleHomeView(
                rule=rule,
