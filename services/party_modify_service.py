...
class PartyModifyService:
    ...
    def remove_party_member_to_waiting(
        ...
        self.waiting_repository.insert_waiting(
            ...
            race=slot["race"] if isinstance(slot, dict) else slot.race,
            server=slot["server"] if isinstance(slot, dict) else slot.server,
    ...
    def add_waiting_member_to_party(
        ...
        self.slot_repository.insert(
            PartySlot(
                ...
                race=waiting["race"] if isinstance(waiting, dict) else waiting.race,
                server=waiting["server"] if isinstance(waiting, dict) else waiting.server,
